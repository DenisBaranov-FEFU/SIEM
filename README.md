...
скрипт alert_mails.py изначально расчитан на почту mail.ru 
для работы алертов необходимо отредактировать сркипт (alert_mails.py) и docker-compose.yml, вписав в последний вместо MAIL и PASSWORD в блоке postfix свои учетные данные 

Для генерации Mail ключа необходимо в почте нажать на аватурку -> Пароли и Безопасность -> В способах входа выбрать "Пароли для внешних приложений" -> Создать пароль с полным доступом (SMTP, IMAP, POP3)
